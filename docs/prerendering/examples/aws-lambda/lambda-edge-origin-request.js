...
// Bound the bot UA in the rewritten querystring; CloudFront's combined URI +
// query has a hard limit and pathological UAs would otherwise push past it.
...
function routeToRenderer(request, targetUrl, userAgent, auth) {
  const safeUserAgent = (userAgent || '').slice(0, MAX_UA_LENGTH);
  request.uri = RENDER_PATH;
  request.querystring = `url=${encodeURIComponent(targetUrl)}&bot=${encodeURIComponent(safeUserAgent)}`;
  request.origin = {
    custom: {
      customHeaders: {},
  ...
  if (safeUserAgent) setHeader(request.headers, 'user-agent', 'User-Agent', safeUserAgent);
  // request.origin was just replaced above, so the customHeaders cleanup inside
  // cleanupInternalHeaders is a no-op here; we still need the request.headers
  // deletes so X-Ostr-* never reach render.ostr.io.
...
async function handler(event) {
  ...
  try {
    ...
    console.warn('[ostr.io] Unexpected Lambda@Edge event shape:', err && err.message);
    throw err;
  }
  ...
  try {
    ...
    console.warn('[ostr.io] pre-rendering route failed; passing request to origin:', err && err.message);
    cleanupInternalHeaders(request);
    return request;
  }
}
